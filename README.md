...
```
├── Assignment.md                  # official assignment instructions (do not edit after submission)
├── README.md                      # this file
├── data/
...
│   └── vizutils.py                # shared layout-audit + label-de-overlap helpers
├── exploration/
│   └── candidates/                # 17 exploration-stage figures (working artifacts,
│                                  #   not report figures — see report Appendix B)
├── images/
...
Requires Python 3 with `pandas`, `numpy`, `scipy`, `scikit-learn`,
`matplotlib` (and `python-docx` for the report build):
